...
function isMatch(search: string, item: string, lastMatchWasAlone = false) {
  if (search.length === 0) {
    return true;
  }
  ...
  for (const idx of idxs) {
    if (lastMatchWasAlone && idx !== 0) {
      continue;
    }
    const res = isMatch(search.slice(1), item.slice(idx + 1), idx !== 0);
    if (res) return true;
  }
...
const res = searchMatch("decojson", [
